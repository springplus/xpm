...
//    $scope.index = viewProvider().getUrl($state.current.data);
    ...
    viewProvider().renderAll(cfgLoadFromRemote, $scope, $$Data)
    /**
     *
    ...
    $scope.configView = function (srcAliasStr, changedAliasStr) {
        ...
        for (var i in viewConfig.views) {
            if ($scope.cfg.views[i].alias == srcAliasStr) {
                ...
                $scope[targetView.renderTo] = viewProvider().getUrl(targetView)
                break;
            }
        }
    }
...
//                var ctrl = $injector.invoke(cfg.name,this,{owner:$scope[alias]});
            var ctrl = undefined;
            try {
                ctrl = eval(cfg.name + "($scope,$$Data)");
            } catch (e) {
                console.warn(">>>未配置视图的同名函数:", cfg.name)
            }
            if (ctrl!=undefined) {
                console.debug(">>>loadData(cfg, $scope, $$Data);>>>",cfg)

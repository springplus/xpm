...
appUtils.provider('$$appRouter', function $$appRouterProvider($stateProvider) {
    ...
    this.setResState = function (resConfig) {
        try {
            ...
            var moduleStateMapping = this.parser().parseAll(resConfig.moduleName, resConfig.entityName)
            ...
            var listStateMapping = this.parser().parseAll(resConfig.moduleName, resConfig.entityName, resConfig.list.view)
            log(listStateMapping);
            console.debug("- mustacheUrl（采用）")
            var listViews = {};
            listViews[listStateMapping.view] = {templateProvider: function ($http) {
                return tmpl_crud_view(listStateMapping.mustacheUrl, $http, resConfig);
            }}
            ...
            for (var viewGroupCode in resConfig.detailViews) {
                var viewGroup = resConfig.detailViews[viewGroupCode];
                for (var viewIndex in viewGroup) {
                    ...
                    if (viewGroup[viewIndex].templateData) {
                        ...
                        var mustacheStateMapping = this.parser().parseAll(resConfig.moduleName, resConfig.entityName, resConfig.list.view, viewGroupCode, viewGroup[viewIndex].fileName)
                        ...
                        var htmlStateMapping = this.parser().parseAll(resConfig.moduleName, resConfig.entityName, resConfig.list.view, viewGroupCode, viewGroup[viewIndex].fileName)
        ...
        function checkEntityConfig(resConfig) {
            ...
            if (!ec) {
                ...
                logErrorMsg('resConfig的属性moduleName、entityName、list、detailViews不能为空。');
                return false;
            } else {
                var viewIsOk = true;
        ...
        function setDefault(resConfig) {
            ...
            if (!resConfig.list.actions) {
                resConfig.list.actions = {};
            }
            if (!resConfig.list.actions.default) {
                resConfig.list.actions.default = defaultActions;
            } else {

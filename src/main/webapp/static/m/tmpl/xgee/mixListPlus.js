 ...
 * @param config
 */
function mixListPlusCtrlTmpl($scope,$$Data, moduleData, $$stateProxy, config) {
    //query
    $scope.xquery = "";
    $scope.query = {};
    $scope.xitems = [
...
function mixListPlusFindActiveDetailView(config, containerName) {
    try {
        if (!containerName) {
            console.error("未配置containerName：" + containerName + ",请检查controller中是否有传该参数，如mixListPlusDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config, \"tabs\")。")
            return undefined;
        }
...
function mixListPlusStepsDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config) {
    return _mixListPlusDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config, "steps");
}
function mixListPlusTabsDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config) {
    return _mixListPlusDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config, "tabs");
}
function mixListPlusNoneDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config) {
    return _mixListPlusDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config, "none");
}
function _mixListPlusDetailCtrlTmpl($scope,$$Data, moduleData, $stateParams, config, containerName) {
    var __moduleName = config.moduleName;
    var __resName = config.resName;
    var activeView = null;
    ...
    function init() {
        activeView = mixListPlusFindActiveDetailView(config, containerName)
    ...
    $scope.onSelectChange = function(data){
        $scope.item[data.uuid] = data.selectedValueStr;
    }

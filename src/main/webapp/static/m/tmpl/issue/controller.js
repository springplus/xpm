...
function issueMainMixListDetailCtrl($scope, $http,$routeParams) {
    if ($routeParams.id)
        $http.get("m/tmpl/issue/data/issue" + $scope.currentId + ".json").success(function (data, status) {
            $scope.issue = data;
        });
...
function issueListCtrl($scope, $http) {
